...
class InviteEarnProvider extends ChangeNotifier {
    ...
    try {
      ...
      await _firestore.runTransaction((tx) async {
        ...
        if (!referrerSnap.exists) {
          throw _InviteException('Invalid referral code');
        }

        final selfSnap = await tx.get(selfRef);
        if (!selfSnap.exists) {
          throw _InviteException('Something went wrong. Please try again.');
        }
        ...
        if (existingRef != null && existingRef.isNotEmpty) {
          throw _InviteException('You have already used a referral code');
        }
