...
import 'model/browse_site_data.dart';
import 'widgets/quest_dialog.dart';
import 'widgets/browse_site_card.dart';

class BrowseSiteScreen extends StatefulWidget {
  const BrowseSiteScreen({super.key});

  @override
  State<BrowseSiteScreen> createState() => _BrowseSiteScreenState();
}

class _BrowseSiteScreenState extends State<BrowseSiteScreen>
    with WidgetsBindingObserver {
  final List<BrowseSiteData> websites = [
    BrowseSiteData(
  ...
  final List<String> websitesUrl = [
    RemoteSettingsService.instance.techNewsUrl,
    RemoteSettingsService.instance.viralGadgetUrl,
    RemoteSettingsService.instance.lifestyleBlogUrl,
    RemoteSettingsService.instance.travelDailiesUrl,
    RemoteSettingsService.instance.financeTipsUrl,
  ];

  Timer? _lockRefreshTimer;
  late final BrowseSiteProvider _provider;
    ...
    return ChangeNotifierProvider.value(
      ...
      child: PopScope(
        ...
        child: Scaffold(
          ...
          appBar: SharedAppBar(title: context.l10n.visitWebsites),
          body: SafeArea(
            child: Consumer<BrowseSiteProvider>(
              builder: (context, provider, _) {
                return ListView.separated(
                  padding: EdgeInsets.symmetric(
                      horizontal: AppDimens.w20, vertical: AppDimens.h20),
                  itemCount: websites.length,
                  separatorBuilder: (_, _) => SizedBox(height: AppDimens.h16),
                          ...
                          Positioned.fill(
                            child: Container(
                              decoration: BoxDecoration(
                                ...
                                borderRadius:
                                    BorderRadius.circular(AppDimens.r14),
                              ),
                              child: Column(
