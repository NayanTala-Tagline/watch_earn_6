/*
import 'dart:async';
import 'package:btc_mining_tracker/utils/logger_ex.dart';
import 'package:btc_mining_tracker/utils/remote_settings_service.dart';
...
class AdClickManager {
  static final AdClickManager _instance = AdClickManager._internal();

  factory AdClickManager() => _instance;

  AdClickManager._internal();
  ...
  Future<void> _loadAndShowAd(
    ...
    'Show Overlay'.logD;
    final overlay = LoadingShade.instance();

    try {
       if (RemoteSettingsService.instance.appShowInter) {
        overlay.show(context: context);
        'try cache'.logD;
        interstitialAdManager?.dispose();

        interstitialAdManager = InterstitialAdManager(
          isEnabled: RemoteSettingsService.instance.appShowInter,
          // Updated to use your Remote Config
          adUnitId: RemoteSettingsService.instance.appInter,
